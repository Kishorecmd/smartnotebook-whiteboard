...
export const PenFamilyPicker: React.FC = () => {
  ...
  const PenButton: React.FC<{ pen: PenPreset }> = ({ pen }) => {
    ...
    return (
      <button
        ...
        className={`pen-family-button relative flex min-h-[var(--pen-target-size)] min-w-0 flex-col items-center justify-center gap-1 rounded-2xl px-1.5 py-1.5 transition-all ${
          isActive
      ...
      >
        ...
        <span className="pen-family-label text-[11px] font-semibold leading-none">{pen.name}</span>
        {isActive && (
  ...
  return (
    <div className="pen-family-picker rounded-3xl border border-slate-700/60 bg-slate-900/95 p-3 shadow-2xl backdrop-blur-xl">
      <div className="mb-2 px-1 text-[11px] font-bold uppercase tracking-wider text-slate-400">
        {childFriendlyMode ? 'Pens' : 'Quick'}
      </div>
      <div className="pen-family-grid">
        {quick.map((pen) => (
